...
class Edit extends React.Component {
  constructor(props) {
    ...
    this.state = {
      content: this.props.task_content,
      duedate: this.props.task_duedate,
      date: ""
  ...
  // componentDidMount() {
    ...
    //   .then(resp => {
      ...
      handleOnSubmit = (values) => {
        ...
        axios.patch(`http://localhost:3000/tasks/${this.props.task_id}`, data, {
        },)
      console.log(data)
    }
            ...
            >
            {
              ({ handleSubmit, handleChange, handleBlur, handleCancel, values, errors, touched, setFieldValue }) => (
                <Form>
                  ...
                  <FormGroup></FormGroup>
                  {/* <Link href="/" className="bg-green-700 font-semibold text-white py-2 px-4 rounded">編集する</Link> */}
                  <Button onClick={handleSubmit} className="bg-green-700 font-semibold text-white py-2 px-4 rounded">
                    <Link href="/">
                      編集する
                    </Link>
                  </Button>
                  {/* <Button onClick={handleSubmit} className="ml-10 bg-red-700 font-semibold text-white py-2 px-4 rounded">編集する</Button> */}
                  <Link href="/">
                    <button className="ml-10 bg-red-700 font-semibold text-white py-2 px-4 rounded">
                      キャンセル
                    </button>
                  </Link>
                </Form>
